...
def simulate(model, model_ids, data):
    ...
    for i in range(4):
        ...
        for i in range(len(arms_list) - 1):
            for j in range(200):
                data.ctrl[arms_dict[arms_list[(i + 2) % 4][0]].motor3.id] = .4
                mujoco.mj_step(model, data)
                viewer.sync()
                time.sleep(timestep)
            moveLimb(arms_dict[arms_list[i][0]], model, data, viewer, data.qpos[model.jnt_qposadr[arms_dict[arms_list[i][0]].revolute1]] - np.pi/4, data.qpos[model.jnt_qposadr[arms_dict[arms_list[i][0]].revolute2]], 0.5)
            for j in range(200):
...
def main():
    with open("BearsQuadrupedLocomotion/Model/ReachBot.Xml") as file:
        xml = file.read()
